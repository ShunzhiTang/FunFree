...
+ (instancetype)tabBarControllerWithViewControllers:(NSArray<UIViewController *> *)viewControllers tabBarItemsAttributes:(NSArray<NSDictionary *> *)tabBarItemsAttributes{
    ...
    FFTabBarController *tabBarController = [[FFTabBarController alloc] initWithViewControllers:viewControllers tabBarItemsAttributes:tabBarItemsAttributes];
...
- (void)setViewControllers:(NSArray *)viewControllers{
    ...
    if (viewControllers && [viewControllers isKindOfClass:[NSArray class]]) {
        ...
        for (UIViewController *viewController in self.viewControllers) {
            ...
            // 在这里添加的时间就是使用的运行时去处理的，改变运行时执行（tabBarController）的方法 ，用tsz_tabBarController去代替进行关联
