...
- (UIViewController *)tsz_popSelectTabBarChildViewControllerAtIndex:(NSUInteger)index {
    ...
    BOOL isNavigationController =
        [[selectedTabBarChildViewController class] isSubclassOfClass:[UINavigationController class]];
    // 如果当前选中的是 navgationController 就返回UINavigationController
    // 的第一个viewControiller
...
- (void)tsz_popSelectTabBarChildViewControllerAtIndex:(NSUInteger)index
                                           completion:(FFPopSelectTabBarChildViewControllerCompletion)completion {
...
- (UIViewController *)tsz_popSelectTabBarChildViewControllerForClassType:(Class)classType {
    ...
    [tabBarController.viewControllers
        enumerateObjectsUsingBlock:^(UIViewController *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
            ...
            BOOL isNavController =
                [[tabBarController.viewControllers[idx] class] isSubclassOfClass:[UINavigationController class]];
...
- (void)tsz_popSelectTabBarChildViewControllerForClassType:(Class)classType
                                                completion:(FFPopSelectTabBarChildViewControllerCompletion)completion {

    UIViewController *selectedTabBarChildViewController =
        [self tsz_popSelectTabBarChildViewControllerForClassType:classType];
...
- (void)checkTabBarChildControllerValidityAtIndex:(NSUInteger)index {
    ...
    @try {
        ...
        NSString *reason = [NSString stringWithFormat:formatString, @(__PRETTY_FUNCTION__), @(__LINE__)];
